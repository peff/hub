...
func (updater *Updater) PromptForUpdate() (err error) {
	...
	if version != "" && version != updater.CurrentVersion {
		...
		default:
			console.Infoln("There is a newer version of hub available.")
			console.Infof("Would you like to update? ([Y]es/[N]o/[A]lways/N[e]ver): ")
			var confirm string
			fmt.Scan(&confirm)
...
func (updater *Updater) Update() (err error) {
	config := autoUpdateConfig()
	if config == "never" {
		console.Infoln("Update is disabled")
		return
	}

	releaseName, version := updater.latestReleaseNameAndVersion()
	if version == "" {
		console.Infoln("There is no newer version of hub available.")
		return
	}

	if version == updater.CurrentVersion {
		console.Infof("You're already on the latest version: %s\n", version)
	} else {
		err = updater.updateTo(releaseName, version)
	}
...
func (updater *Updater) updateTo(releaseName, version string) (err error) {
	console.Infof("Updating gh to %s...\n", version)
